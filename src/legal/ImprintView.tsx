  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4 md:px-8 text-left space-y-8 animate-fade-in">
      <div className="border-b border-[var(--border-subtle)] pb-6">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-[var(--text-primary)]">Legal Notice / Imprint</h1>
      ...
      {!LEGAL_CONFIG.isConfigurationComplete && (
        <div className="p-4 rounded-sm bg-[#171A20] border border-[#2D333F] flex items-start gap-3">
          ...
          <div className="text-xs text-[var(--text-tertiary)] space-y-1">
            <span className="font-semibold text-[var(--text-primary)]">Statutory Compliance Disclosure:</span>
            <p>
              The business registry, address, and VAT entity details below reflect the configuration template. Any values marked <code className="text-[var(--accent-lime)] bg-[#111317] px-1 py-0.5 rounded">REQUIRED_CONFIGURATION</code> must be configured via production environment variables prior to commercial deployment.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
        <div className="p-5 rounded-sm bg-[#121418] border border-[var(--border-subtle)] space-y-3">
          ...
          <div className="space-y-1.5 text-[var(--text-secondary)]">
            ...
            <div><strong className="text-[var(--text-primary)]">Country:</strong> {LEGAL_CONFIG.country}</div>
            <div><strong className="text-[var(--text-primary)]">Commercial Register ID:</strong> {LEGAL_CONFIG.registrationNumber}</div>
        ...
        <div className="p-5 rounded-sm bg-[#121418] border border-[var(--border-subtle)] space-y-3">
          ...
          <div className="space-y-1.5 text-[var(--text-secondary)]">
            ...
            <div><strong className="text-[var(--text-primary)]">Data Protection Officer:</strong> <a href={`mailto:${LEGAL_CONFIG.privacyEmail}`} className="text-[var(--accent-lime)]">{LEGAL_CONFIG.privacyEmail}</a></div>
            <div><strong className="text-[var(--text-primary)]">Platform URL:</strong> https://masteringlocal.pro</div>
          </div>
        </div>
      </div>
      ...
      <section className="space-y-3 text-sm text-[var(--text-secondary)] leading-relaxed">
        ...
        <p className="text-xs text-[var(--text-tertiary)]">
          The European Commission provides an online dispute resolution platform for consumers, which can be found at <a href="https://ec.europa.eu/consumers/odr" target="_blank" rel="noopener noreferrer" className="text-[var(--accent-lime)] underline">https://ec.europa.eu/consumers/odr</a>. We are not obliged and generally not willing to participate in dispute settlement proceedings before a consumer arbitration board.
        </p>
      </section>
    </div>
  );
