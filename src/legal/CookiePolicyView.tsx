  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4 md:px-8 text-left space-y-8 animate-fade-in">
      <div className="border-b border-[#242830] pb-6">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-[#F4F3EF]">Cookie Policy</h1>
      ...
      <div className="p-5 rounded-xl bg-[#121418] border border-[#2A2E35] space-y-3">
        <div className="flex items-center gap-2 text-sm font-semibold text-[#F4F3EF]">
          <Shield className="w-4 h-4 text-[#D6AF62]" />
          <span>Core Privacy Principle</span>
        </div>
        <p className="text-xs text-[#A0A6B2] leading-relaxed">
          MasteringLocal.Pro is architected around local-first processing. Your audio files are processed inside your browser&apos;s WebAssembly / Web Audio runtime and are <strong className="text-[#F4F3EF]">never uploaded to our servers</strong>. Cookies and local storage mechanisms on this website are strictly limited to session security, quota management, and optional preferences.
        </p>
        <div className="pt-2">
          <button
            type="button"
            onClick={handleOpenConsentModal}
            className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-[#1F232B] hover:bg-[#282F3A] border border-[#343B48] text-xs font-semibold text-[#F4F3EF] transition-colors"
          >
            <Settings className="w-3.5 h-3.5 text-[#D6AF62]" />
      ...
      <section className="space-y-3 text-sm text-[#A0A6B2] leading-relaxed">
        <h2 className="text-lg font-semibold text-[#F4F3EF]">1. What are Cookies and Local Storage?</h2>
        <p>
      ...
      <section className="space-y-4 text-sm text-[#A0A6B2] leading-relaxed">
        <h2 className="text-lg font-semibold text-[#F4F3EF]">2. Inventory of Used Storage & Cookies</h2>
        ...
        <div className="overflow-x-auto border border-[#242830] rounded-xl bg-[#0E1013]">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-[#242830] bg-[#14171D] text-[#F4F3EF]">
            ...
            <tbody className="divide-y divide-[#1D2128]">
              {ACTUAL_COOKIE_INVENTORY.map((item) => (
                <tr key={item.name} className="hover:bg-[#12151B] transition-colors">
                  <td className="p-3 font-mono text-[#D6AF62] text-[11px] whitespace-nowrap">{item.name}</td>
                  <td className="p-3 capitalize text-[#F4F3EF]">
      ...
      <section className="space-y-3 text-sm text-[#A0A6B2] leading-relaxed">
        <h2 className="text-lg font-semibold text-[#F4F3EF]">3. How to Manage or Revoke Your Consent</h2>
        <p>
      ...
      <section className="p-4 rounded-lg bg-[#14171D] border border-[#242830] text-xs text-[#8E95A2] space-y-1">
        <div className="font-semibold text-[#F4F3EF]">Data Protection Questions</div>
        <div>Entity: {LEGAL_CONFIG.businessName}</div>
        <div>Support & Privacy Contact: <a href={`mailto:${LEGAL_CONFIG.supportEmail}`} className="text-[#D6AF62] underline">{LEGAL_CONFIG.supportEmail}</a></div>
      </section>
    </div>
  );
};
