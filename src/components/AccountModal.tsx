  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--bg-primary)]/85 backdrop-blur-md animate-fade-in">
      <div className="bg-[var(--bg-secondary)] border border-[var(--border-subtle)] rounded-sm w-full max-w-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {/* Tab Navigation */}
        <div className="flex items-center px-6 border-b border-[var(--border-subtle)] bg-[var(--bg-secondary)] text-xs font-mono overflow-x-auto">
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-4 flex-1 text-xs">
          {actionMessage && (
            <div
              className={`p-3 rounded-sm border flex items-center gap-2 text-xs ${
                actionMessage.type === 'success'
                  ? 'bg-[#1C170E] border-[var(--accent-lime)] text-[var(--accent-lime)]'
          ...
          {activeTab === 'subscription' && (
            <div className="space-y-4">
              <div className="bg-[var(--bg-primary)] p-4 rounded-sm border border-[var(--border-subtle)] space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-[10px] text-[var(--text-tertiary)] uppercase font-mono block">CURRENT TIER</span>
                    <h4 className="text-base font-semibold text-[var(--text-primary)] flex items-center gap-2 mt-0.5">
                      {currentPlanConfig.name}
                      <ProBadge size="xs" locked={!isPro} />
                    </h4>
                  </div>
                  <span
                    className={`px-2.5 py-1 rounded-sm text-[10px] font-mono font-semibold border ${
                      isPro
                        ? entitlement.cancelAtPeriodEnd
                          ? 'bg-[#1C170E] text-[var(--accent-lime-hover)] border-[var(--accent-lime-hover)]/40'
                          : 'bg-[#1C170E] text-[var(--accent-lime)] border-[var(--accent-lime)]/40'
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-[11px] font-mono">
                  <div>
                    <span className="text-[var(--text-tertiary)] block text-[10px]">INTERVAL</span>
                    <span className="text-[var(--text-primary)]">{currentPlanConfig.periodLabel || 'Forever'}</span>
                  </div>
                  <div>
                    <span className="text-[var(--text-tertiary)] block text-[10px]">PRICE</span>
                    <span className="text-[var(--text-primary)]">€{currentPlanConfig.price} / cycle</span>
                  </div>
                  <div>
                    <span className="text-[var(--text-tertiary)] block text-[10px]">
                      {entitlement.cancelAtPeriodEnd ? 'ACCESS EXPIRES' : 'NEXT RENEWAL'}
                    </span>
                    <span className="text-[var(--text-primary)]">{formattedPeriodEnd}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                {isPro ? (
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <ExternalLink className="w-3.5 h-3.5 text-[var(--accent-lime)]" />
                    ...
                    {entitlement.cancelAtPeriodEnd ? (
                      ...
                      <button
                        ...
                        disabled={isProcessing}
                        className="px-4 py-2 bg-[var(--bg-elevated)] hover:bg-[#1B1F24] text-[#E56B6B] border border-[#E56B6B]/30 font-medium font-mono rounded-sm text-xs transition cursor-pointer flex items-center gap-1.5"
                      >
          ...
          {activeTab === 'billing' && (
            <div className="space-y-4">
              ...
              {/* Payment Method */}
              <div className="bg-[var(--bg-primary)] p-4 rounded-sm border border-[var(--border-subtle)] space-y-2">
                <span className="text-[10px] text-[var(--text-tertiary)] uppercase font-mono block">SAVED PAYMENT METHOD</span>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="px-2.5 py-1.5 rounded bg-[var(--bg-elevated)] border border-[var(--border-subtle)] text-[var(--accent-lime)] font-mono font-bold text-xs">
                  ...
                  <button
                    type="button"
                    onClick={handleOpenStripePortal}
                    className="text-[10px] text-[var(--accent-lime)] hover:underline flex items-center gap-1 font-mono"
                  >
              ...
              <div className="space-y-2">
                <span className="text-xs font-semibold text-[var(--text-primary)] font-mono">Billing History &amp; Invoices</span>
                {invoices.length === 0 ? (
                  <div className="p-6 text-center text-[var(--text-secondary)] bg-[var(--bg-primary)] rounded-sm border border-[var(--border-subtle)] font-mono text-xs">
                    No invoices generated yet for this account.
                  </div>
                ) : (
                  <div className="divide-y divide-[var(--border-subtle)] bg-[var(--bg-primary)] rounded-sm border border-[var(--border-subtle)] overflow-hidden font-mono">
                    {invoices.map((inv) => (
                      <div key={inv.id} className="p-3 flex items-center justify-between text-xs">
                        <div className="space-y-0.5">
                          <span className="font-semibold text-[var(--text-primary)]">{inv.number}</span>
                          <span className="text-[10px] text-[var(--text-secondary)] block">
                            {new Date(inv.created).toLocaleDateString()} · {inv.interval}ly
                          </span>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="font-semibold text-[var(--text-primary)]">€{inv.amount.toFixed(2)}</span>
          ...
          {activeTab === 'usage' && (
            <div className="space-y-4">
              ...
              <div className="bg-[var(--bg-primary)] p-4 rounded-sm border border-[var(--border-subtle)] space-y-2 text-[11px] text-[var(--text-secondary)]">
                <strong className="text-[var(--text-primary)] font-mono">Tier Entitlement Limits:</strong>
          ...
          {activeTab === 'exports' && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-[var(--text-primary)] font-mono">Recent Master Audio Exports</span>
                <span className="text-[10px] text-[var(--text-secondary)] font-mono">{exportHistory.length} tracks rendered</span>
              </div>
                ...
                <div className="divide-y divide-[var(--border-subtle)] bg-[var(--bg-primary)] rounded-sm border border-[var(--border-subtle)] overflow-hidden font-mono">
                  {exportHistory.map((exp) => (
                    <div key={exp.id} className="p-3 flex items-center justify-between text-xs">
                      <div className="space-y-0.5">
                        <span className="font-semibold text-[var(--text-primary)]">{exp.filename}</span>
                        <div className="flex items-center gap-2 text-[10px] text-[var(--text-secondary)]">
                          <span>{exp.format}</span>
                          <span>•</span>
                          <span>{new Date(exp.createdAt).toLocaleDateString()}</span>
                          <span>•</span>
                          <span className="text-[var(--accent-lime)]">{exp.profileName}</span>
                        </div>
                      </div>
                      <span className="px-2 py-0.5 rounded bg-[var(--bg-elevated)] text-[var(--accent-lime)] text-[10px] font-bold border border-[var(--border-subtle)]">
                        {exp.tier}
                      </span>
                    </div>
                  ))}
          ...
          {activeTab === 'privacy' && (
            <div className="space-y-4 text-xs">
              <div className="p-4 rounded-sm bg-[var(--bg-primary)] border border-[var(--border-subtle)] space-y-2 text-[var(--text-secondary)]">
                ...
                {onNavigateToDataRequest && (
                  <button
                    ...
                    className="text-[var(--accent-lime)] underline font-medium block pt-1"
                  >
              ...
              {/* Account Deletion */}
              <div className="p-4 rounded-sm bg-[#171012] border border-[#3D1E22] space-y-3">
                ...
                {!showDeleteConfirm ? (
                  <button
                    ...
                    className="px-3 py-2 bg-[#2D161A] hover:bg-[#3D1E24] text-[#E56B6B] border border-[#E56B6B]/40 rounded-sm text-xs font-mono transition cursor-pointer flex items-center gap-2"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                    Request Account Deletion
                  </button>
                ) : (
                  <div className="p-3 bg-[#0E0C0D] rounded-sm border border-[#E56B6B]/40 space-y-2">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-1.5 bg-[#171012] border border-[#3D1E22] rounded text-xs text-[var(--text-primary)] outline-none"
                    />
                    <div className="flex items-center gap-2 pt-1">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-[#171A20] text-[var(--text-secondary)] rounded text-xs"
                      >
                        Cancel
                      </button>
                    </div>
        ...
        {/* Footer */}
        <div className="px-6 py-3 border-t border-[var(--border-subtle)] bg-[var(--bg-primary)] flex items-center justify-between text-[11px] text-[var(--text-tertiary)] font-mono">
