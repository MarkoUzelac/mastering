      ...
      <div className="bg-[#0E1116] border border-[#1E2530] rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#1E2530] bg-[#0A0C0F]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-semibold text-[#F4F3EF]">EBU R128 Loudness Telemetry</h3>
              <p className="text-[11px] text-[#646A73]">ITU-R BS.1770-4 calibrated audio metering</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="bg-[#07090C] border border-[#181C22] rounded-xl p-4 flex items-center justify-between">
            <div>
              <span className="text-[10px] font-mono uppercase tracking-wider text-[#646A73]">
                Program Loudness (Integrated)
              </span>
              <div className="text-3xl font-mono font-bold text-[#F4F3EF] mt-1">
                {integrated.toFixed(1)}{' '}
                <span className="text-sm font-semibold text-[#8B5CF6]">LUFS</span>
              </div>
            </div>

            <div className="text-right">
              <div className="text-[11px] font-mono text-[#646A73]">Target: {targetLufs.toFixed(1)} LUFS</div>
              <div
                className={`text-xs font-mono font-semibold mt-1 ${
                  Math.abs(delta) <= 1.0 ? 'text-[#10B981]' : delta > 0 ? 'text-[#F59E0B]' : 'text-[#8B5CF6]'
                }`}
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[#07090C] border border-[#181C22] rounded-xl p-3">
              <div className="text-[10px] font-mono text-[#646A73]">Momentary Max</div>
              <div className="text-base font-mono font-bold text-[#E5E7EB] mt-1">
                {shortTerm.toFixed(1)} LUFS
              </div>
              <div className="text-[10px] text-[#646A73] mt-0.5">400ms sliding RMS window</div>
            </div>

            <div className="bg-[#07090C] border border-[#181C22] rounded-xl p-3">
              <div className="text-[10px] font-mono text-[#646A73]">Loudness Range (LRA)</div>
              <div className="text-base font-mono font-bold text-[#E5E7EB] mt-1">{lra.toFixed(1)} LU</div>
              <div className="text-[10px] text-[#646A73] mt-0.5">Dynamic variation statistical span</div>
            </div>

            <div className="bg-[#07090C] border border-[#181C22] rounded-xl p-3">
              <div className="text-[10px] font-mono text-[#646A73]">Max True Peak</div>
              <div className="text-base font-mono font-bold text-[#E5E7EB] mt-1">
                {truePeak.toFixed(1)} dBTP
              </div>
              <div className="text-[10px] text-[#646A73] mt-0.5">4x inter-sample peak detector</div>
            </div>

            <div className="bg-[#07090C] border border-[#181C22] rounded-xl p-3">
              <div className="text-[10px] font-mono text-[#646A73]">Crest Factor</div>
              <div className="text-base font-mono font-bold text-[#E5E7EB] mt-1">
                {crestFactor.toFixed(1)} dB
              </div>
              <div className="text-[10px] text-[#646A73] mt-0.5">Peak-to-RMS transient ratio</div>
            </div>
          </div>
        </div>
