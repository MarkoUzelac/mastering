...
async function runGemini(input: string, schema: Record<string, unknown>) {
  ...
  if (!apiKey) {
    throw new Error('AI provider is not configured. Set GEMINI_API_KEY on the server.');
  }
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    if (path.endsWith('/mastering')) {
      ...
      const result = await runGemini(prompt, masteringSchema);
    ...
    if (path.endsWith('/release')) {
      ...
      const prompt = `${RELEASE_SYSTEM}\n\nCreate a release package from this data:\n${JSON.stringify({
        ...
        mixingEngineer: body.mixingEngineer || '',
        masteringEngineer: body.masteringEngineer || '',
        copyright: body.copyright || '',
      })}`;
      const result = await runGemini(prompt, releaseSchema);
      return setJson(res, 200, result);
    }
    ...
    return setJson(res, 500, { error: message });
  }
}
